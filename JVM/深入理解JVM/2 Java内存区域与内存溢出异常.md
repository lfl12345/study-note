...
字节码解释器的工作就是通过改变程序计数器中的值来选去下一条需要执行的字节码指令，它是程序控制流的指示器，分支、循环、跳转、异常处理、线程恢复等基础功能都需要依赖程序计数器来完成。
...
局部变量表所需要的**槽的数量**（槽的大小并不确定，根据虚拟机）在编译器完成分配，当进入一个方法的时候，这个方法需要在栈帧中分配多大的局部变量空间是完全确定的。
...
Java堆是虚拟机所管理的内存中最大的一块；Java堆是被多有线程共享的一块内存区域，在虚拟机启动的时候创建。
...
一下对象的探究只包含java的普通对象，通过new创建，不包括数组、Class对象以及通过复制反序列化创建的对象。
...
5. 在上面工作完成之后，从jvm的角度来看，一个新的对象以及产生了，但是从java程序的角度来看，对象创建才刚刚开始——构造函数，即Class文件中的\<init>()方法还没有执行，所有的字段都是默认值。
